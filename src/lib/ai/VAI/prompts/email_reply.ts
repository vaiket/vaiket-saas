// prompts/email_reply.ts
export const placeholderPromptEmail = `export interface EmailReplyPromptInput {
subject: string;
text: string;
...
export function buildEmailReplyPrompt({ subject, text, context, tenantName }: EmailReplyPromptInput) {
...
3. If the user asks for sensitive actions (refunds, login, billing, OTP, account deletion, address change), reply with: "REQUIRES_HUMAN".
4. Do NOT guess. If information is missing, politely ask.
5. Keep the reply short unless user asks for detailed explanation.
