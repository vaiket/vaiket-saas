...
interface Props {
  onVerified?: () => void; // 🔥 IMPORTANT
}

/* ================= COMPONENT ================= */

export default function DomainAuthenticationCard({ onVerified }: Props) {
  ...
  const loadFromDB = async () => {
    ...
    try {
      ...
      if (data?.dns?.length > 0) {
        const row = data.dns[0];
        setDns(row);

        // 🔥 FINAL LOGIC: if all verified → unlock next step
        if (
          row.spfStatus === "verified" &&
          row.dkimStatus === "verified" &&
          row.dmarcStatus === "verified"
        ) {
          onVerified?.(); // ✅ THIS FIXES EVERYTHING
        }
      } else {
        setDns(null);
      }
